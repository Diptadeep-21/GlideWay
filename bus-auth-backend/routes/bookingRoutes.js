  ...
  try {
    ...
    if (req.user.role === 'driver') {
      ...
      bookings = await Booking.find({ busId: { $in: busIds } })
        .select('busId userId seatsBooked totalFare travelDate status contactDetails passengers fareBreakdown trackingLink boardingPoint actualDepartureTime actualArrivalTime earnings isChatEnabled cancellationReason isGroupBooking allowSocialTravel ratingId amenityRatings delayNotice')
        .populate({
          path: 'busId',
    ...
    res.json({
      ...
      bookings: validBookings.map(b => ({
        ...
        busDetails: b.busId || {},
        busId: b.busId?._id || null,
        driverId: b.busId?.driverId?._id || null,
        driverName: b.busId?.driverId?.name || 'N/A',
        fareBreakdown: b.fareBreakdown,
        boardingPoint: b.boardingPoint,
        trackingLink: b.trackingLink,
        isChatEnabled: b.isChatEnabled,
        actualDepartureTime: b.actualDepartureTime?.toISOString(),
        actualArrivalTime: b.actualArrivalTime?.toISOString(),
        earnings: b.earnings,
        cancellationReason: b.cancellationReason,
        allowSocialTravel: b.allowSocialTravel,
         delayNotice: b.delayNotice,
      })),
    });
  } catch (err) {
