...
const ChatWithDriver = ({ bookingId, userId }) => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      console.log('Fetched messages for ChatWithDriver:', response.data);
      ...
      console.error('Fetch failed in ChatWithDriver:', error.message);
  ...
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      console.log('Socket connected in ChatWithDriver:', newSocket.id);
      newSocket.emit('joinRoom', { bookingId });
      fetchMessages();
    });

    const handleIncoming = (msg) => {
      console.log('Incoming message in ChatWithDriver:', msg);
      if (!msg.message?.trim()) {
    ...
    newSocket.on('error', (errorData) => {
      console.error('Socket error in ChatWithDriver:', errorData);
      setError(errorData.message || 'Failed to send message');
    });
    newSocket.on('connect_error', (error) => {
      console.error('Socket connect_error in ChatWithDriver:', error);
    });
    newSocket.on('disconnect', (reason) => {
      console.log('Socket disconnected in ChatWithDriver:', reason);
      if (reason === 'io server disconnect') {
        newSocket.connect();
      }
    });
    newSocket.io.on('close', (reason, description) => {
      console.log('Socket.IO underlying connection closed in ChatWithDriver:', reason, description);
    });
  ...
  return (
    <div className="w-full bg-white dark:bg-neutral-800 rounded-lg shadow-lg p-4">
      <h3 className="text-lg font-bold text-neutral-800 dark:text-neutral-50 mb-4">
        Chat with Driver for Booking {bookingId}
      </h3>
